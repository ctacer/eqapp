var FilterSample={FREQ_MUL:7000,QUAL_MUL:30,playing:false};
FilterSample.play=function(){
	var source=context.createBufferSource();
	source.buffer=BUFFERS.techno;
	var filter=context.createBiquadFilter();
	filter.type=0;filter.frequency.value=5000;
	source.connect(filter);filter.connect(context.destination);
	if(!source.start)
		source.start=source.noteOn;
	source.start(0);source.loop=true;
...
FilterSample.changeFrequency=function(element){
	var minValue=40;var maxValue=context.sampleRate/2;
...
FilterSample.toggleFilter=function(element){
	this.source.disconnect(0);this.filter.disconnect(0);
	if(element.checked){this.source.connect(this.filter);
